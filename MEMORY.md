...
**Dernière mise à jour** : 2026-05-29 (fix affichage photos questionnaire sur fiche client : referrerpolicy Drive + section ouverte + labels) + avenant contrat + méthode prog IA
...
- **Photos du questionnaire d'intégration** : questions de type `photo` (custom, q_integration_v2). Uploadées sur Google Drive (Apps Script `DRIVE_UPLOAD_URL`), URLs stockées dans `questionnaires.extra_questions[questionId]` (tableau d'URLs). Affichées sur la fiche client → onglet Questionnaire → section « Questions personnalisées » (rendu par `_qRenderAllAnswers`). Labels mappés via `_qCustomLabelMap` (chargé depuis settings.q_integration + q_integration_v2.custom dans `openDet`).
